...
def read_code(d, k, seed, c_type='sur', n=None):
    if c_type == 'qcc':
        ...
        return torch.load(
            abspath(dirname(__file__)).strip('module') + 'code/' + c_type + '_d{}_k{}_seed{}'.format(d, k, seed))


def read_data(d, k, seed, c_type='sur'):
    return torch.load(
        abspath(dirname(__file__)).strip('module') + 'database/' + c_type + '_d{}_k{}_seed{}.pt'.format(d, k, seed))
...
class Errormodel():
    ...
    def ising_error(self, n_s, n=13, degree=4, beta=0.3, h=1.0):
        _, s = torch.load(
            abspath(dirname(__file__)).strip('module') + 'database/ising/' + 'n{}_degree{}_beta{}_h{}.pt'.format(n,
                                                                                                                 degree,
                                                                                                                 beta,
                                                                                                                 h))
        # print(s)
        ...
        if multi == False:
            ...
            for i in range(self.L ** 2):
                if i % 2 == 1:
                    ...
                    multi_opt = torch.cat([opt] * bdim, dim=0).view([bdim * (2 ** l),
                                                                     -1])  # torch.tensor([opt.tolist()]*bdim, device=self.device).view([bdim*(2**l), -1])
