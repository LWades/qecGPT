...
hx, hz = hx.cpu().numpy(), hz.cpu().numpy()
console.print("Hx:")
console.print(hx)
console.print("Hz:")
console.print(hz)
...
l = np.zeros_like(l1)
l[:, :n], l[:, n:] = l1[:, n:], l1[:, :n]   # 重排列 l1，重排列逻辑算符，懂？
# print(l)
console.print(l)
...
if e_model == 'depolarized':
    if k == 1:
        error_rate = torch.tensor([0.25])
        console.print(error_rate.shape)
...
for i in range(len(error_rate) if e_model == 'depolarized' else len(beta_list)):
    ...
    syndrome = mod2.commute(errors, code.g_stabilizer)  # 错误症状：错误 * 稳定子生成矩阵
    pe = E.pure(code.pure_es, syndrome, device=device, dtype=dtype) # 纯错误：纯错误矩阵 * 错误症状
    console.print("pure_es: \n{}".format(code.pure_es))
    console.print("errors(before): ")
    console.print(errors)
    console.print("syndrome(before): ")
    console.print(syndrome)
    console.print("pe: ")
    console.print(pe)
    ...
    correct_number = 0
    t = 0
    checks = torch.empty(trials, code.n * 2)
    for j in range(trials):
        console.print("Num {} decoding: ".format(j+1))
        e = error[j]
        console.print("error: ")
        console.print(e)
        s = syndrome[j]
        ...
        console.print(recover)
        console.print("type e: {}".format(type(e)))
        console.print("type recover: {}".format(type(recover)))
        check = (e + recover) % 2
        console.print("check: (shape={})".format(check.shape))
        console.print(check)
        checks[j] = torch.tensor(check)
        console.print("check commute with g_stabilizer: \n{}".format(mod2.commute(torch.tensor(check), code.PCM)))
        s = np.sum((check @ l.T) % 2)   # 看有没有奇数个逻辑错误(mod 2）
        console.print("s: ")
        console.print(s)
        t2 = time.time()
        t = t + (t2 - t1)
    ...
    console.print("logic errors: \n{}".format(mod2.commute(mod2.xyz(checks), code.logical_opt)))
